# Server MQTT (Internet of Things)

Dengan mengaktifkan opsi [open_mqtt_protocol](/server/setting?id=open_mqtt_protocol), sistem bakal parse header `MQTT`, dan event [onReceive](/server/events?id=onreceive) di Worker process bakal ngembaliin satu paket data `MQTT` yang utuh tiap kali dipanggil.

Kamu bisa pake Swoole sebagai server atau client MQTT buat implementasi solusi Internet of Things (IOT) yang lengkap.

> Buat parsing protokol MQTT yang lengkap dan coroutine client, bisa pake [simps/mqtt](https://github.com/simps/mqtt)

## Kode Program

Tulis kode berikut ke dalam mqttServer.php
...
$server->set([
    'open_mqtt_protocol' => true, // Aktifkan protokol MQTT
    'worker_num' => 1,
]);
