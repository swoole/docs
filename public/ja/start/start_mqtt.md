...
[open_mqtt_protocol](/server/setting?id=open_mqtt_protocol)オプションを設定すると、`MQTT`パケットのヘッダを解析し、Workerプロセスの[onReceive](/server/events?id=onreceive)イベントは毎回完全な`MQTT`データパケットを返します。

Swooleを使用すると、MQTTサーバーやクライアントを実現し、IoT（インターネット・オブ・シングス）の完全なソリューションを構築することができます。

> 完全なMQTTプロトコルの解析と协程クライアントは[simps/mqtt](https://github.com/simps/mqtt)を使用できます。

## プログラムコード

以下のコードをmqttServer.phpに書き込んでください。
